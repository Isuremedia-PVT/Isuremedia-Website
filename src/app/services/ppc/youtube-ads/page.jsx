...
const BRIDGE_ITEMS = [
  'YouTube Ads strategy and full campaign setup',
...
const CREATIVE = [
  { icon: 'fa-solid fa-stopwatch', title: 'Hook before the skip button appears', impact: 'Highest impact', desc: 'Skippable in-stream ads are decided in five seconds. We build every video around a hook that earns the rest of the watch.' },
  { icon: 'fa-solid fa-link', title: 'Message match between ad and landing page', impact: 'High impact', desc: 'The ad creates an expectation. Any gap between what it promises and what the page delivers kills conversion rates.' },
  { icon: 'fa-solid fa-flask', title: 'Systematic creative testing', impact: 'High impact', desc: 'We test hooks, formats, lengths, and calls to action in a structured way, building a picture of what your audience responds to.' },
  { icon: 'fa-solid fa-arrows-rotate', title: 'Fresh creative on rotation', impact: 'Ongoing', desc: 'Ad fatigue is real and measurable on video. We rotate new creative regularly to keep performance and CPVs controlled.' },
];
...
const PROCESS = [
  { n: '01', title: 'Audit & Goal Alignment', desc: 'We audit your account or start fresh, reviewing conversion tracking and any existing video assets before building anything.' },
  { n: '02', title: 'Strategy & Campaign Build', desc: 'Objectives, campaign types, and audiences mapped, with GA4 and Google Ads tracking verified before launch.' },
  { n: '03', title: 'Creative Development & Launch', desc: 'Hooks, scripting guidance, and video direction developed around the strongest hypothesis, with a testing plan from day one.' },
  { n: '04', title: 'Creative Testing', desc: 'In the first four to six weeks we systematically test hooks, formats, and calls to action to find what converts.' },
  { n: '05', title: 'Scaling & Ongoing Management', desc: 'Budget scaled into what performs, with new creative rotated and remarketing expanded as view data grows.' },
  { n: '06', title: 'Monthly Report & Review', desc: 'Performance, creative rotation needs, budget allocation, and the plan for the following month.' },
];

const FAQS = [
  { q: 'Do I need a professionally produced video to run YouTube Ads?', a: 'No. Well-scripted, phone-shot video often outperforms polished production if the hook and message are strong. We advise on what actually needs a bigger budget.' },
  { q: 'How much should I spend on YouTube Ads?', a: 'Most small and medium businesses start between $1,000 and $3,000 a month and scale based on cost per view and conversion results.' },
  { q: 'How long does it take to see results?', a: 'Most campaigns show clear view and click trends within the first two weeks, with conversion data solidifying by month two.' },
  { q: 'What is the difference between in-stream, in-feed, and Shorts ads?', a: 'In-stream plays before or during other videos, in-feed appears in search and recommended results, and Shorts run in the vertical short-form feed. We use a mix based on your goals.' },
...
const RELATED = [
  ...
  { icon: 'fa-brands fa-meta', title: 'Meta Ads', desc: 'Facebook and Instagram campaigns built on precise audience targeting and creative testing.', href: '/services/ppc/meta-ads' },
  ...
  return (
    <section style={{ padding: '100px 0', background: 'var(--color-bg-soft)' }}>
      <div className="ism-container">
        <div className="yads-faq-grid" style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 64, alignItems: 'start' }}>
          <div className="yads-faq-sticky" style={{ position: 'sticky', top: 100 }}>
            <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,40px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', marginBottom: 14, marginTop: 0, lineHeight: 1.15 }}>
              Questions About <span style={{ color: 'var(--ism-amber)' }}>YouTube Ads</span>
            </h2>
      ...
      <style>{`
        @media (max-width: 900px) {
          .yads-faq-grid { grid-template-columns: minmax(0,1fr) !important; gap: 32px !important; }
          .yads-faq-sticky { position: static !important; }
        }
      `}</style>
    </section>
  );
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* ══ 01. HERO ══════════════════════════════════════════════ */}
        <section className="yads-hero" style={{ background: 'linear-gradient(160deg,var(--ism-blue-50) 0%,#fff 60%)', padding: '88px 0 96px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: '-15%', right: '-8%', width: 720, height: 720, background: 'radial-gradient(circle,rgba(30,77,195,.07) 0%,transparent 65%)', pointerEvents: 'none' }} />

          <div className="ism-container">
            <div className="yads-hero-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.95fr)', gap: 56, alignItems: 'center', position: 'relative', zIndex: 1 }}>
              ...
              <div className="yads-hero-photo" style={{ position: 'relative' }}>
                ...
                <img src="/banner/youtube ads.webp" alt="YouTube Ads management" style={{ width: '100%', height: 440, objectFit: 'contain', background: 'transparent', borderRadius: 24, display: 'block' }} />
                ...
                <div style={{ position: 'absolute', top: 26, right: -20, background: '#fff', borderRadius: 14, padding: '12px 18px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 10 }}>
                  <i className="fa-solid fa-google" style={{ color: 'var(--ism-amber)', fontSize: 14 }} />
                  <span style={{ fontFamily: J, fontSize: 13, fontWeight: 700, color: 'var(--color-navy)' }}>Google Ads Ecosystem</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: 28, left: -24, background: '#fff', borderRadius: 14, padding: '12px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)' }}>
                  ...
                  <span style={{ fontFamily: I, fontSize: 11, fontWeight: 600, color: 'var(--color-text-muted)' }}>Campaigns Live &amp; Converting</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: -16, right: 12, background: '#fff', borderRadius: 14, padding: '10px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 12, fontWeight: 700, color: '#0E9B6E' }}>2.5B+ Monthly Viewers</span>
                </div>
              </div>
          ...
          <style>{`
            @media (max-width: 900px) {
              .yads-hero-grid { grid-template-columns: minmax(0,1fr) !important; gap: 60px !important; }
              .yads-hero-photo { margin: 0 12px 24px; }
            }
            @media (max-width: 480px) {
              .yads-hero { padding: 48px 0 64px !important; }
            }
          `}</style>
        </section>
            ...
            <div className="bridge-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,0.9fr)', gap: 64, alignItems: 'start' }}>
              <div>
                <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(24px,2.8vw,40px)', color: 'var(--color-navy)', lineHeight: 1.2, letterSpacing: '-0.4px', marginBottom: 24 }}>
                  A Video Nobody Watches Past Five Seconds Isn&apos;t an Ad. It&apos;s a Skipped Impression.
                </h2>
                ...
                <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.82, marginBottom: 32 }}>
                  ...
                  built to be watched, not just to be shown.
                </p>
                <Link href="/contact"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '15px 32px', borderRadius: 8, fontFamily: J, fontSize: 14, fontWeight: 700, color: 'var(--color-navy)', background: 'var(--ism-amber)', textDecoration: 'none', letterSpacing: '.04em', textTransform: 'uppercase', boxShadow: '0 6px 20px rgba(255,176,0,.35)', transition: 'all .18s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-accent-hover)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'var(--ism-amber)'; e.currentTarget.style.transform = ''; }}>
                  Claim Your Free YouTube Ads Audit
                </Link>
              </div>
        ...
        {/* ══ 04. WHAT IS YOUTUBE ADS MANAGEMENT ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div className="yads-whatis-box" style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 24, padding: '56px 56px', boxShadow: '0 24px 64px rgba(0,35,83,.08)' }}>
            <div className="yads-whatis-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.85fr)', gap: 64, alignItems: 'center' }}>
              <div>
                <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', lineHeight: 1.22, letterSpacing: '-0.4px', marginBottom: 20 }}>
                  Reach People While They&apos;re Already Searching for a Solution.
                </h2>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
                    YouTube is the second-largest search engine in the world, with{' '}
                    intent already present in how and what people search for.
                  </p>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
                    With over 2.5 billion monthly logged-in users, YouTube gives businesses access to{' '}
                    Google-level targeting in video form, in-market, affinity, and remarketing audiences.
                  </p>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
              ...
              <div className="yads-score-wrap" style={{ position: 'relative', height: 340, borderRadius: 20, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'visible' }}>
                ...
                <div style={{ position: 'relative', width: 168, height: 168, borderRadius: '50%', background: 'conic-gradient(var(--ism-amber) 0deg 324deg, var(--ism-blue-100) 324deg 360deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 14px 38px rgba(0,35,83,.14)' }}>
                  <div style={{ width: 134, height: 134, borderRadius: '50%', background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontFamily: J, fontSize: 34, fontWeight: 900, color: 'var(--color-navy)', lineHeight: 1 }}>2.5B+</span>
                    <span style={{ fontFamily: J, fontSize: 10, fontWeight: 700, color: 'var(--color-text-muted)', letterSpacing: '.05em', textTransform: 'uppercase', marginTop: 4 }}>Monthly Viewers</span>
                  </div>
                </div>

                <div className="yads-score-badge" style={{ position: 'absolute', top: 18, left: 0, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-crosshairs" style={{ color: 'var(--color-primary)', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Audience</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Defined</div>
                  </div>
                </div>

                <div className="yads-score-badge" style={{ position: 'absolute', top: 30, right: -6, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'rgba(255,176,0,.16)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-clapperboard" style={{ color: 'var(--ism-amber)', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Creative</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Tested</div>
                  </div>
                </div>

                <div className="yads-score-badge" style={{ position: 'absolute', bottom: 8, left: -10, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Tracking</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Active</div>
                  </div>
                </div>

                <div className="yads-score-badge" style={{ position: 'absolute', bottom: 24, right: 4, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-rotate-left" style={{ color: 'var(--color-primary)', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Remarketing</div>
          ...
          <style>{`
            @media(max-width:860px){ .yads-whatis-grid{ grid-template-columns:minmax(0,1fr) !important; gap:40px !important; } .yads-whatis-grid > div:last-child{ order:-1; } }
            @media(max-width:640px){ .yads-whatis-box{ padding:32px 24px !important; } }
            @media(max-width:500px){ .yads-score-badge{ padding:8px 10px !important; gap:7px !important; } .yads-score-badge > div:first-child{ width:24px !important; height:24px !important; } }
          `}</style>
        </section>

        {/* ══ 05. WHY YOUTUBE ADS WORKS ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 18 }}>Reach 2.5 Billion Viewers With the Precision of Google Search.</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.78 }}>
                Video Action campaigns paired with remarketing consistently produce some of the strongest cost-per-result numbers across the Google Ads ecosystem.
              </p>
            </div>
            ...
            <div style={{ textAlign: 'center' }}>
              <Link href="/contact"
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-accent-hover)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'var(--ism-amber)'; e.currentTarget.style.transform = ''; }}>
                Start Reaching Your Audience on YouTube
              </Link>
            </div>
          </div>
          <style>{`
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Who YouTube Ads Management Is Built For</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>If Your Customers Watch Video Before They Buy, This Is Where We Find Them.</p>
            </div>
          ...
          <div className="ism-container">
            <div className="mid-cta" style={{ background: 'var(--color-primary)', borderRadius: 20, padding: '40px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', maxWidth: 560 }}>
                <h3 style={{ fontFamily: J, fontSize: 'clamp(18px,2vw,24px)', fontWeight: 800, color: '#fff', marginBottom: 8, lineHeight: 1.3 }}>Find out what a properly managed YouTube campaign could deliver.</h3>
                <p style={{ fontFamily: I, fontSize: 14.5, color: 'rgba(255,255,255,.80)', lineHeight: 1.65, margin: 0 }}>
                  A free audit will show you what your current creative and targeting are missing.
                </p>
              </div>
              <Link href="/contact" className="mid-cta-btn"
                ...
                Run My Free YouTube Ads Audit <i className="fa-solid fa-arrow-right" style={{ fontSize: 11 }} />
              </Link>
            </div>
          </div>
          <style>{`
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Why Businesses Choose Isuremedia for YouTube Ads</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>Hook-Led Creative. Full-Funnel Strategy. Results You Can See.</p>
            </div>
            <div className="why-ism-bento" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16 }}>
              ...
              <div className="why-ism-bento-row" style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                ...
                <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', minHeight: 200 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img loading="lazy" src="\services-mid-image\ppc.webp" alt="What makes Isuremedia different" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                </div>
        ...
        {/* ══ 11. OUR PROCESS ══════════════════════════════════════════════ */}
        <section className="yads-section" style={{ padding: '100px 0', background: '#fff' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 64 }}>
              <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,44px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', margin: '0 0 14px' }}>
                How ISureMedia Manages <span style={{ color: 'var(--ism-amber)' }}>YouTube Ads Campaigns</span>
              </h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75, margin: 0 }}>
                Audit. Build. Test. Optimise. Scale.
              </p>
            </div>
            <div className="yads-timeline" style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 0, position: 'relative' }}>
            ...
            <p style={{ textAlign: 'center', fontFamily: I, fontSize: 14, color: 'var(--color-text-muted)', marginTop: 40 }}>
              Most campaigns show clear view and click trends within two weeks, with conversion data solidifying by month two to three.
            </p>
          ...
          <style>{`
            @media (max-width: 900px) {
              .yads-timeline { grid-template-columns: repeat(3,1fr) !important; gap: 40px !important; }
            }
            @media (max-width: 560px) {
              .yads-timeline { grid-template-columns: 1fr !important; }
              .yads-timeline > *:not(:last-child)::after { content:''; display:block; width:2px; height:32px; background:var(--ism-amber); margin:20px auto 0; }
            }
          `}</style>
        </section>
        ...
        <section style={{ padding: '96px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <p style={{ fontFamily: I, fontSize: 15, color: 'var(--color-text-muted)', marginTop: 10 }}>A few things that go well with YouTube Ads.</p>
            </div>
            <div className="yads-related-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
              {RELATED.map((r, i) => (
          ...
          <style>{`@media(max-width:860px){ .yads-related-grid{ grid-template-columns:1fr !important; } }`}</style>
        </section>
