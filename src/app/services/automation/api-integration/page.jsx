...
const BRIDGE_ITEMS = [
  'API integration strategy and data mapping',
  'Two-way sync between your CRM, tools, and platforms',
...
const FAILURE_MODES = [
  ...
  { icon: 'fa-solid fa-screwdriver-wrench', title: 'No one monitoring it after launch', impact: 'Ongoing', desc: 'Platforms update their APIs. An integration that isnâ€™t monitored eventually breaks silently, and nobody knows until data goes missing.' },
];
...
const SERVICES = [
  ...
  { icon: 'fa-solid fa-bolt', title: 'Webhook Configuration', desc: 'Real-time, event-driven data flow instead of scheduled batch syncs that lag behind reality.' },
  { icon: 'fa-solid fa-shield-halved', title: 'Error Handling & Data Validation', desc: 'Logic that catches failed syncs and malformed data before they cause downstream problems.' },
  { icon: 'fa-solid fa-gears', title: 'Middleware Setup', desc: 'Zapier, Make, or custom middleware configured to orchestrate multi-step data flows.' },
  { icon: 'fa-solid fa-chart-simple', title: 'Ongoing Monitoring & Maintenance', desc: 'Integrations checked and updated as platforms change their APIs, so connections keep working.' },
];
...
const WHO_FOR = [
  ...
  { icon: 'fa-solid fa-building', title: 'Businesses running multiple CRMs or tools', desc: 'Sales, support, and marketing teams using different platforms need data flowing between them automatically.', img: '/services-six-card/Marketing Automation Section/API Integration/Businesses running multiple CRMs or tools.webp' },
  { icon: 'fa-solid fa-file-invoice-dollar', title: 'Businesses with manual data entry bottlenecks', desc: 'If someone is copy-pasting data between systems daily, that role has just become an automation project.', img: '/services-six-card/Marketing Automation Section/API Integration/Businesses with manual data entry bottlenecks.webp' },
  { icon: 'fa-solid fa-puzzle-piece', title: 'Businesses using niche or custom software', desc: 'Platforms without a native integration ecosystem still need to connect, custom API work bridges the gap.', img: '/services-six-card/Marketing Automation Section/API Integration/Businesses using niche or custom software.webp' },
  { icon: 'fa-solid fa-arrow-trend-up', title: 'Businesses scaling past spreadsheet workflows', desc: 'What worked at a smaller size breaks down as volume grows. Proper integrations replace fragile manual processes.', img: '/services-six-card/Marketing Automation Section/API Integration/Businesses scaling past spreadsheet workflows.webp' },
  { icon: 'fa-solid fa-triangle-exclamation', title: 'Businesses that have been burned by data errors', desc: 'Missed orders, duplicate contacts, and mismatched records caused by manual transfer are a solvable problem.', img: '/services-six-card/Marketing Automation Section/API Integration/Businesses that have been burned by data errors.webp' },
];
...
const WHY_ISM = [
  ...
  { icon: 'fa-solid fa-code', title: 'We build custom where native doesnâ€™t exist', desc: 'Not limited to off-the-shelf integrations, we write custom API connections for the tools your business actually runs on.' },
...
const PROCESS = [
  { n: '01', title: 'Audit & Discovery', desc: 'We map your current tool stack, data flow, and the specific gaps causing manual work or data mismatches.' },
  { n: '02', title: 'Data Mapping & Architecture', desc: 'Field-by-field mapping across systems, and a decision on native integration, middleware, or custom API build.' },
  { n: '03', title: 'Integration Build', desc: 'Connections built and configured, with webhook triggers and error-handling logic built in from the start.' },
  { n: '04', title: 'Testing & Validation', desc: 'Every data flow tested end to end, including edge cases and failure scenarios, before going live.' },
  { n: '05', title: 'Launch & Monitoring Setup', desc: 'Integrations go live with monitoring in place to catch failed syncs or API changes early.' },
  { n: '06', title: 'Ongoing Maintenance', desc: 'Regular checks and updates as platforms change their APIs, keeping every connection working reliably.' },
];
...
const FAQS = [
  ...
  { q: 'What is the difference between a native integration and a custom API build?', a: 'A native integration uses a pre-built connection the platform already offers. A custom build is written specifically for tools that donâ€™t have one, using each platformâ€™s API directly.' },
  ...
  { q: 'How long does an integration project take?', a: 'Simple integrations can be live within days. Custom, multi-system builds with complex data mapping typically take two to four weeks.' },
  { q: 'Can you fix an integration someone else built that keeps failing?', a: 'Yes. We regularly audit and rebuild integrations that fail silently or were never built with proper error handling.' },
  { q: 'Is my data secure during integration?', a: 'Yes. We use secure authentication methods (OAuth, API keys) and never store sensitive data outside the systems it belongs in.' },
  { q: 'Do you provide documentation after the build?', a: 'Yes. Every integration comes with clear documentation of what connects to what, how data flows, and how to troubleshoot common issues.' },
];
  ...
  return (
    <section style={{ padding: '100px 0', background: 'var(--color-bg-soft)' }}>
      <div className="ism-container">
        <div className="api-faq-grid" style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 64, alignItems: 'start' }}>
          <div className="api-faq-sticky" style={{ position: 'sticky', top: 100 }}>
            <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,40px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', marginBottom: 14, marginTop: 0, lineHeight: 1.15 }}>
              Questions About <span style={{ color: 'var(--ism-amber)' }}>API Integration</span>
            </h2>
      ...
      <style>{`
        @media (max-width: 900px) {
          .api-faq-grid { grid-template-columns: minmax(0,1fr) !important; gap: 32px !important; }
          .api-faq-sticky { position: static !important; }
        }
      `}</style>
    </section>
  );
  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="api-hero" style={{ background: 'linear-gradient(160deg,var(--ism-blue-50) 0%,#fff 60%)', padding: '88px 0 96px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: '-15%', right: '-8%', width: 720, height: 720, background: 'radial-gradient(circle,rgba(30,77,195,.07) 0%,transparent 65%)', pointerEvents: 'none' }} />

          <div className="ism-container">
            <div className="api-hero-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.95fr)', gap: 56, alignItems: 'center', position: 'relative', zIndex: 1 }}>
              ...
              <div className="api-hero-photo" style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', top: 26, right: -20, background: '#fff', borderRadius: 14, padding: '12px 18px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 13, fontWeight: 700, color: 'var(--color-navy)' }}>Real-Time Sync</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: 28, left: -24, background: '#fff', borderRadius: 14, padding: '12px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)' }}>
                  ...
                  <span style={{ fontFamily: I, fontSize: 11, fontWeight: 600, color: 'var(--color-text-muted)' }}>Systems Connected &amp; Synced</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: -16, right: 12, background: '#fff', borderRadius: 14, padding: '10px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 12, fontWeight: 700, color: '#0E9B6E' }}>Zero Manual Entry</span>
                </div>
              </div>
          ...
          <style>{`
            @media (max-width: 900px) {
              .api-hero-grid { grid-template-columns: minmax(0,1fr) !important; gap: 60px !important; }
              .api-hero-photo { margin: 0 12px 24px; }
            }
            @media (max-width: 480px) {
              .api-hero { padding: 48px 0 64px !important; }
            }
          `}</style>
        </section>
            ...
            <div className="bridge-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,0.9fr)', gap: 64, alignItems: 'start' }}>
              <div>
                ...
                <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.82, marginBottom: 32 }}>
                  ...
                  mapped, tested, and monitored, so data moves automatically and accurately.
                </p>
                <Link href="/contact"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '15px 32px', borderRadius: 8, fontFamily: J, fontSize: 14, fontWeight: 700, color: 'var(--color-navy)', background: 'var(--ism-amber)', textDecoration: 'none', letterSpacing: '.04em', textTransform: 'uppercase', boxShadow: '0 6px 20px rgba(255,176,0,.35)', transition: 'all .18s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-accent-hover)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'var(--ism-amber)'; e.currentTarget.style.transform = ''; }}>
                  Claim Your Free Integration Audit
                </Link>
              </div>
        ...
        {/* â•â• 04. WHAT IS API INTEGRATION â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div className="api-whatis-box" style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 24, padding: '56px 56px', boxShadow: '0 24px 64px rgba(0,35,83,.08)' }}>
            <div className="api-whatis-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.85fr)', gap: 64, alignItems: 'center' }}>
              <div>
                <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', lineHeight: 1.22, letterSpacing: '-0.4px', marginBottom: 20 }}>
                  One Source of Truth, Across Every Tool You Use.
                </h2>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
                    API integration means your platforms{' '}
                    update each other automatically, no exports, no imports, no manual reconciliation.
                  </p>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
                    Where native integrations don&apos;t exist, we build{' '}
                    custom API connections directly against each platform&apos;s own API.
                  </p>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
                    The result is a business where every system reflects the same reality, in real time, without anyone doing it by hand.
                  </p>
                </div>
              </div>
              <div className="api-score-wrap" style={{ position: 'relative', height: 340, borderRadius: 20, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'visible' }}>
                <div aria-hidden style={{ position: 'absolute', width: 260, height: 260, borderRadius: '50%', border: '1px dashed var(--ism-blue-100)' }} />

                <div style={{ position: 'relative', width: 168, height: 168, borderRadius: '50%', background: 'conic-gradient(var(--ism-amber) 0deg 360deg, var(--ism-blue-100) 360deg 360deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 14px 38px rgba(0,35,83,.14)' }}>
                  <div style={{ width: 134, height: 134, borderRadius: '50%', background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontFamily: J, fontSize: 34, fontWeight: 900, color: 'var(--color-navy)', lineHeight: 1 }}>0</span>
                    <span style={{ fontFamily: J, fontSize: 10, fontWeight: 700, color: 'var(--color-text-muted)', letterSpacing: '.05em', textTransform: 'uppercase', marginTop: 4 }}>Manual Entries</span>
                  </div>
                </div>

                <div className="api-score-badge" style={{ position: 'absolute', top: 18, left: 0, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Data Mapped</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Complete</div>
                  </div>
                </div>

                <div className="api-score-badge" style={{ position: 'absolute', top: 30, right: -6, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Webhooks</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Live</div>
                  </div>
                </div>

                <div className="api-score-badge" style={{ position: 'absolute', bottom: 8, left: -10, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                ...
                <div className="api-score-badge" style={{ position: 'absolute', bottom: 24, right: 4, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-chart-simple" style={{ color: 'var(--color-primary)', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Monitoring</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Ongoing</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <style>{`
            @media(max-width:860px){ .api-whatis-grid{ grid-template-columns:minmax(0,1fr) !important; gap:40px !important; } .api-whatis-grid > div:last-child{ order:-1; } }
            @media(max-width:640px){ .api-whatis-box{ padding:32px 24px !important; } }
            @media(max-width:500px){ .api-score-badge{ padding:8px 10px !important; gap:7px !important; } .api-score-badge > div:first-child{ width:24px !important; height:24px !important; } }
          `}</style>
        </section>

        {/* â•â• 05. WHY API INTEGRATION MATTERS â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 18 }}>Every System Working From the Same Data, Automatically.</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.78 }}>
                Properly connected systems eliminate the manual work and human error that come from moving data by hand.
              </p>
            </div>
        ...
        {/* â•â• 06. WHAT CAUSES INTEGRATIONS TO FAIL â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
        ...
        {/* â•â• 07. OUR API INTEGRATION SERVICES â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Who API Integration Is Built For</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>If Your Tools Don&apos;t Talk to Each Other, This Is What Fixes That.</p>
            </div>
          ...
          <div className="ism-container">
            <div className="mid-cta" style={{ background: 'var(--color-primary)', borderRadius: 20, padding: '40px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', maxWidth: 560 }}>
                <h3 style={{ fontFamily: J, fontSize: 'clamp(18px,2vw,24px)', fontWeight: 800, color: '#fff', marginBottom: 8, lineHeight: 1.3 }}>Find out how much manual work your integrations could eliminate.</h3>
                <p style={{ fontFamily: I, fontSize: 14.5, color: 'rgba(255,255,255,.80)', lineHeight: 1.65, margin: 0 }}>
                  A free audit will show you exactly where your data is breaking down.
                </p>
              </div>
              <Link href="/contact" className="mid-cta-btn"
                ...
                Run My Free Integration Audit <i className="fa-solid fa-arrow-right" style={{ fontSize: 11 }} />
              </Link>
            </div>
          </div>
          <style>{`
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Why Businesses Choose Isuremedia for API Integration</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>Mapped Properly. Built to Last. Monitored Continuously.</p>
            </div>
        ...
        <section className="api-section" style={{ padding: '100px 0', background: '#fff' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 64 }}>
              <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,44px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', margin: '0 0 14px' }}>
                How ISureMedia Builds <span style={{ color: 'var(--ism-amber)' }}>API Integrations</span>
              </h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75, margin: 0 }}>
                Audit. Map. Build. Test. Monitor.
              </p>
            </div>
            <div className="api-timeline" style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 0, position: 'relative' }}>
            ...
            <p style={{ textAlign: 'center', fontFamily: I, fontSize: 14, color: 'var(--color-text-muted)', marginTop: 40 }}>
              Simple integrations can be live within days. Custom, multi-system builds typically take two to four weeks depending on complexity.
            </p>
          ...
          <style>{`
            @media (max-width: 900px) {
              .api-timeline { grid-template-columns: repeat(3,1fr) !important; gap: 40px !important; }
            }
            @media (max-width: 560px) {
              .api-timeline { grid-template-columns: 1fr !important; }
              .api-timeline > *:not(:last-child)::after { content:''; display:block; width:2px; height:32px; background:var(--ism-amber); margin:20px auto 0; }
            }
          `}</style>
        </section>
        ...
        <section style={{ padding: '96px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <p style={{ fontFamily: I, fontSize: 15, color: 'var(--color-text-muted)', marginTop: 10 }}>A few things that go well with API integration.</p>
            </div>
            <div className="api-related-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
              {RELATED.map((r, i) => (
          ...
          <style>{`@media(max-width:860px){ .api-related-grid{ grid-template-columns:1fr !important; } }`}</style>
        </section>
