...
const BENEFITS = [
  {
    ...
    title: 'Complex Logic Support',
    desc: 'Branching paths, conditional routers, iterators, and aggregators — Make handles multi-path scenarios that Zapier simply cannot.',
  },
  {
    icon: 'fa-solid fa-shuffle',
    title: 'Data Transformation',
    desc: 'Parse, format, and transform data mid-workflow using built-in functions — map JSON, clean strings, calculate values on the fly.',
  },
  {
    icon: 'fa-solid fa-coins',
    title: 'Lower Cost at Scale',
    desc: 'Make charges per operation rather than per Zap, making it significantly cheaper for high-volume or complex automation needs.',
  },
];

const INCLUDED = [
  'Scenario mapping & architecture',
  '5 complex scenarios built',
...
const STEPS = [
  { num: '01', title: 'Map', desc: 'We document every data source, transformation step, and output destination before building a single module.' },
  { num: '02', title: 'Design', desc: 'Scenario architecture designed with error handling, data filters, and conditional paths mapped out clearly.' },
  { num: '03', title: 'Build', desc: 'All scenarios built in Make with testing data, error alerts, and scheduling configured.' },
  { num: '04', title: 'Test & Monitor', desc: 'Full QA run on every scenario path, monitoring alerts set up, and documentation delivered.' },
];

const RELATED = [
  { icon: 'fa-solid fa-plug', title: 'Zapier Automation', desc: 'For simpler, app-to-app automations where speed of setup matters more than advanced logic.', href: '/services/automation/zapier' },
  { icon: 'fa-solid fa-code-branch', title: 'n8n Automation', desc: 'Self-hosted automation for teams that need total control, custom code nodes, and no per-operation pricing.', href: '/services/automation/n8n' },
          ...
          <div className="ism-container">
            <nav style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' as const }}>
              {[
                ...
                { label: 'Make (Integromat) Automation', href: null },
              ].map((crumb, i, arr) => (
        ...
        <section style={{ background: 'var(--color-bg-soft)', padding: '100px 0', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 40px', textAlign: 'center', position: 'relative' }}>
            ...
            <p style={{ fontFamily: I, fontSize: 18, color: 'var(--color-text-muted)', lineHeight: 1.78, maxWidth: 640, margin: '0 auto 44px' }}>
              Advanced automation scenarios built in Make (formerly Integromat) — multi-step, conditional, and data-transform workflows.
            </p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'center', flexWrap: 'wrap' as const }}>
              <a href="/contact" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '15px 32px', borderRadius: 8, fontFamily: J, fontSize: 14, fontWeight: 700, color: 'var(--color-navy)', background: 'var(--ism-amber)', textDecoration: 'none', letterSpacing: '.04em', textTransform: 'uppercase' as const, boxShadow: '0 6px 20px rgba(255,176,0,.35)', transition: 'all .18s' }}
                ...
                Build Make Workflows <i className="fa-solid fa-arrow-right" style={{ fontSize: 11 }} />
              </a>
          ...
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 40px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 72, alignItems: 'center' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {['Multi-branch conditional routing', 'Data aggregation and transformation', 'Scheduled batch operations', 'API calls and webhook processing'].map((item, i) => (
            ...
            <div>
              ...
              <img src="https://images.unsplash.com/photo-1518186285589-2f7649de83e0?w=700&q=80" alt="Make automation workflow design" style={{ width: '100%', borderRadius: 20, boxShadow: '0 24px 72px rgba(30,77,195,.13)', display: 'block' }} />
            </div>
          </div>
        </section>
