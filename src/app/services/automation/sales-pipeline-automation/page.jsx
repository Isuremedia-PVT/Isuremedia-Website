...
const SPA_BRIDGE_ITEMS = [
  'Pipeline audit and stage mapping',
  'Automated stage-change triggers',
  'Task and reminder creation for reps',
  'Follow-up sequencing by deal stage',
  'Deal-aging alerts for stalled opportunities',
  'Forecast and reporting automation',
  'CRM cleanup and field standardisation',
];

const SPA_WHY_MATTERS = [
  { icon: 'fa-solid fa-bell', title: 'No more forgotten follow-ups', desc: 'Every deal gets a task and a reminder the moment it needs one, so a rep having a busy week never means a lead goes cold.' },
  { icon: 'fa-solid fa-clock', title: 'Deals move at the speed of the buyer', desc: 'Stage changes trigger the next action instantly, instead of waiting for someone to remember to update the CRM.' },
  { icon: 'fa-solid fa-chart-line', title: 'Forecasting you can actually trust', desc: 'When every rep updates the pipeline the same way automatically, your forecast reflects reality, not guesswork.' },
  { icon: 'fa-solid fa-users', title: 'Consistent follow-up across every rep', desc: 'New hires and veteran closers alike follow the same automated cadence, so results stop depending on individual discipline.' },
  { icon: 'fa-solid fa-triangle-exclamation', title: 'Stalled deals get flagged, not forgotten', desc: 'Deal-aging alerts surface opportunities sitting untouched, so managers catch the problem before the deal dies quietly.' },
  { icon: 'fa-solid fa-gauge-high', title: 'Less admin, more selling', desc: 'Automated task creation and CRM updates hand reps their next action instead of a data-entry chore.' },
];

const SPA_CARD_VARIANTS = [
...
const SPA_KEY_FACTORS = [
  ...
  { icon: 'fa-solid fa-broom', title: 'CRM Cleanup and Standardisation', impact: 'Medium impact', desc: 'Fields, stages, and naming conventions standardised so automation has consistent data to work from.' },
  { icon: 'fa-solid fa-vial', title: 'Testing and Validation', impact: 'Medium impact', desc: 'Every trigger and sequence tested against real deal scenarios before it goes live on your pipeline.' },
  { icon: 'fa-solid fa-chalkboard-user', title: 'Team Training and Documentation', impact: 'Contextual', desc: 'Clear documentation and a live walkthrough so your team understands exactly how each automation behaves.' },
];
...
const SPA_SERVICES = [
  ...
  { icon: 'fa-solid fa-chart-line', title: 'Forecasting Dashboards', desc: 'Live forecast views built on clean, automatically updated pipeline data your leadership can trust.' },
  { icon: 'fa-solid fa-broom', title: 'CRM Cleanup', desc: 'Stages, fields, and naming conventions standardised so your pipeline data is reliable enough to automate.' },
  { icon: 'fa-solid fa-route', title: 'Lead-to-Deal Handoff Automation', desc: 'Qualified leads convert into pipeline deals automatically, with the right owner and stage assigned instantly.' },
  { icon: 'fa-solid fa-file-lines', title: 'Sales Reporting Automation', desc: 'Recurring pipeline and rep-performance reports generated and delivered without anyone building a spreadsheet.' },
];
...
const SPA_PROCESS = [
  { n: '01', title: 'Audit', desc: 'We map your current pipeline, review deal flow, and find exactly where opportunities stall or get forgotten.' },
  { n: '02', title: 'Map Stages and Triggers', desc: 'Every stage change is mapped to the exact task, alert, or sequence it should fire before we build anything.' },
  { n: '03', title: 'Build Automations', desc: 'Stage triggers, task creation, reminders, and follow-up sequences built directly inside your CRM.' },
  { n: '04', title: 'Test', desc: 'Every trigger and sequence tested against real deal scenarios before it goes live on your pipeline.' },
  { n: '05', title: 'Document and Train', desc: 'Clear documentation delivered and a live team walkthrough so your team can manage it confidently.' },
];
  ...
  return (
    <section style={{ padding: '100px 0', background: 'var(--color-bg-soft)' }}>
      <div className="ism-container">
        <div className="spa-faq-grid" style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 64, alignItems: 'start' }}>
          {/* Left */}
          <div className="spa-faq-sticky" style={{ position: 'sticky', top: 100 }}>
            <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,40px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', marginBottom: 14, marginTop: 0, lineHeight: 1.15 }}>
              Questions About <span style={{ color: 'var(--ism-amber)' }}>Pipeline Automation</span>
            </h2>
      ...
      <style>{`
        @media (max-width: 900px) {
          .spa-faq-grid { grid-template-columns: minmax(0,1fr) !important; gap: 32px !important; }
          .spa-faq-sticky { position: static !important; }
        }
      `}</style>
    </section>
  );
...
export default function SalesPipelineAutomationPage() {
  return (
    <>
      <Navbar />
      <main>
        ...
        <section className="spa-hero" style={{ background: 'linear-gradient(160deg,var(--ism-blue-50) 0%,#fff 60%)', padding: '88px 0 96px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div className="spa-hero-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.95fr)', gap: 56, alignItems: 'center', position: 'relative', zIndex: 1 }}>
              ...
              <div className="spa-hero-photo" style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', top: 26, right: -20, background: '#fff', borderRadius: 14, padding: '12px 18px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 13, fontWeight: 700, color: 'var(--color-navy)' }}>Zero Missed Follow-Ups</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: -16, right: 12, background: '#fff', borderRadius: 14, padding: '10px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 12, fontWeight: 700, color: '#0E9B6E' }}>Pipeline Live</span>
                </div>
              </div>
          ...
          <style>{`
            @media (max-width: 900px) {
              .spa-hero-grid { grid-template-columns: minmax(0,1fr) !important; gap: 60px !important; }
              .spa-hero-photo { margin: 0 12px 24px; }
            }
            @media (max-width: 480px) {
              .spa-hero { padding: 48px 0 64px !important; }
            }
          `}</style>
        </section>
            ...
            <div className="bridge-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,0.9fr)', gap: 64, alignItems: 'start' }}>
              <div>
                <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(24px,2.8vw,40px)', color: 'var(--color-navy)', lineHeight: 1.2, letterSpacing: '-0.4px', marginBottom: 24 }}>
                  Deals Stall Because Follow-Up Depends on Someone Remembering.
                </h2>
                <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.82, marginBottom: 16 }}>
                  Most CRMs are only as good as the people updating them. A deal moves stage and nobody logs it, a follow-up is due and the rep is buried in other calls, a lead goes quiet and no one notices until it is too late. The pipeline looks organised, but revenue is leaking through the cracks.
                </p>
              ...
              <div style={{ background: '#fff', borderRadius: 20, padding: '36px 32px', border: '1px solid var(--color-border)', boxShadow: '0 20px 50px rgba(0,35,83,.08)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {SPA_BRIDGE_ITEMS.map((item, i) => (
                    <div key={item} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '13px 0', borderBottom: i === SPA_BRIDGE_ITEMS.length - 1 ? 'none' : '1px solid var(--color-border)' }}>
        ...
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            ...
            <div className="spa-whatis-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.85fr)', gap: 64, alignItems: 'center' }}>
              <div>
                <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', lineHeight: 1.22, letterSpacing: '-0.4px', marginBottom: 20 }}>
                  The Fastest Way to Stop Losing Deals to Silence.
                </h2>
              ...
              <div className="spa-score-wrap" style={{ position: 'relative', height: 340, borderRadius: 20, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'visible' }}>
                ...
                <div style={{ position: 'relative', width: 168, height: 168, borderRadius: '50%', background: 'conic-gradient(var(--ism-amber) 0deg 328deg, var(--ism-blue-100) 328deg 360deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 14px 38px rgba(0,35,83,.14)' }}>
                  <div style={{ width: 134, height: 134, borderRadius: '50%', background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <span style={{ fontFamily: J, fontSize: 10, fontWeight: 700, color: 'var(--color-text-muted)', letterSpacing: '.05em', textTransform: 'uppercase', marginTop: 4, textAlign: 'center', maxWidth: 96 }}>Deals Lost to Follow-Up Gaps</span>
                  </div>
                </div>
                ...
                <div className="spa-score-badge" style={{ position: 'absolute', top: 18, left: 0, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Stages Automated</div>
                ...
                <div className="spa-score-badge" style={{ position: 'absolute', bottom: 8, left: -10, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Forecast Accuracy</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Improved</div>
                  </div>
                </div>
                ...
                <div className="spa-score-badge" style={{ position: 'absolute', bottom: 24, right: 4, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Rep Admin Time</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Reduced</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <style>{`
            @media(max-width:860px){ .spa-whatis-grid{ grid-template-columns:minmax(0,1fr) !important; gap:40px !important; } .spa-whatis-grid > div:last-child{ order:-1; } }
            @media(max-width:640px){ .spa-whatis-box{ padding:32px 24px !important; } }
            @media(max-width:500px){ .spa-score-badge{ padding:8px 10px !important; gap:7px !important; } .spa-score-badge > div:first-child{ width:24px !important; height:24px !important; } }
          `}</style>
        </section>
        ...
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 18 }}>Why Pipeline Automation Pays for Itself.</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.78 }}>
                A properly automated pipeline stops relying on memory and gives every deal the same shot at closing.
              </p>
            </div>
            <div className="why-matters-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24, marginBottom: 48 }}>
              {SPA_WHY_MATTERS.map((w, i) => {
                const variant = SPA_CARD_VARIANTS[i % 3];
                return (
            ...
            <div style={{ textAlign: 'center' }}>
              <Link href="/contact"
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-accent-hover)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'var(--ism-amber)'; e.currentTarget.style.transform = ''; }}>
                See What Your Pipeline Could Automate
              </Link>
            </div>
          </div>
          <style>{`
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 18 }}>What Goes Into Every Pipeline Automation Build.</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.78 }}>
                A pipeline that runs itself is not an accident. It is deliberate design across audit, stage mapping, trigger building, and testing. These are the factors Isuremedia builds into every engagement.
              </p>
            </div>
            <div className="spa-factor-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 20 }}>
              {SPA_KEY_FACTORS.map((f, i) => {
                const hl = i === 0;
                return (
                  <div key={f.title} className={hl ? 'spa-factor-card spa-factor-card-hl' : 'spa-factor-card'} style={{ background: hl ? 'linear-gradient(135deg,#1840A0,#2F5FE8)' : '#fff', borderRadius: 16, padding: '28px 26px', border: hl ? 'none' : '1px solid var(--color-border)', boxShadow: hl ? '0 16px 36px rgba(24,64,160,.28)' : 'none', transition: 'transform .2s ease, box-shadow .2s ease, background .25s ease, border-color .25s ease' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 12 }}>
                      <div style={{ width: 46, height: 46, position: 'relative', flexShrink: 0 }}>
                        <div className="spa-factor-card-diamond" style={{ position: 'absolute', inset: 0, borderRadius: 12, border: `2px solid ${hl ? 'rgba(255,255,255,.5)' : 'var(--ism-blue-100)'}`, transform: 'rotate(45deg)', transition: 'border-color .25s ease' }} />
                        <div style={{ position: 'relative', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <i className={`spa-factor-card-icon ${f.icon}`} style={{ color: hl ? '#fff' : 'var(--color-primary)', fontSize: 17, transition: 'color .25s ease' }} />
                        </div>
                      </div>
                      <div>
                        <h3 className="spa-factor-card-title" style={{ fontFamily: J, fontSize: 16, fontWeight: 700, color: hl ? '#fff' : 'var(--color-navy)', margin: 0, lineHeight: 1.3, transition: 'color .25s ease' }}>{f.title}</h3>
                        <span className="spa-factor-card-badge" style={{ fontFamily: J, fontSize: 10.5, fontWeight: 700, color: hl ? 'var(--ism-amber)' : 'var(--color-accent-hover)', letterSpacing: '.05em', textTransform: 'uppercase', transition: 'color .25s ease' }}>{f.impact}</span>
                      </div>
                    </div>
                    <p className="spa-factor-card-desc" style={{ fontFamily: I, fontSize: 14, color: hl ? 'rgba(255,255,255,.85)' : 'var(--color-text-muted)', lineHeight: 1.72, margin: 0, transition: 'color .25s ease' }}>{f.desc}</p>
                  </div>
                );
              })}
            </div>
          ...
          <style>{`
            .spa-factor-card:not(.spa-factor-card-hl):hover{ transform: translateY(-4px); box-shadow: 0 16px 36px rgba(24,64,160,.28) !important; background: linear-gradient(135deg,#1840A0,#2F5FE8) !important; border-color: transparent !important; }
            .spa-factor-card:not(.spa-factor-card-hl):hover .spa-factor-card-diamond{ border-color: rgba(255,255,255,.5) !important; }
            .spa-factor-card:not(.spa-factor-card-hl):hover .spa-factor-card-icon{ color: #fff !important; }
            .spa-factor-card:not(.spa-factor-card-hl):hover .spa-factor-card-title{ color: #fff !important; }
            .spa-factor-card:not(.spa-factor-card-hl):hover .spa-factor-card-badge{ color: var(--ism-amber) !important; }
            .spa-factor-card:not(.spa-factor-card-hl):hover .spa-factor-card-desc{ color: rgba(255,255,255,.85) !important; }
            .spa-factor-card-hl:hover{ transform: translateY(-4px); box-shadow: 0 20px 44px rgba(24,64,160,.36); }
            @media (max-width:700px){ .spa-factor-grid{ grid-template-columns:1fr !important; } }
          `}</style>
        </section>
        ...
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Our Sales Pipeline Automation Services</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>Everything It Takes to Keep Every Deal Moving.</p>
            </div>
        ...
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Who Sales Pipeline Automation Is Built For</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>If Deals Go Cold Because Follow-Up Depends on Memory, This Is for You.</p>
            </div>
          ...
          <div className="ism-container">
            <div className="mid-cta" style={{ background: 'var(--color-primary)', borderRadius: 20, padding: '40px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', maxWidth: 560 }}>
                <h3 style={{ fontFamily: J, fontSize: 'clamp(18px,2vw,24px)', fontWeight: 800, color: '#fff', marginBottom: 8, lineHeight: 1.3 }}>Your pipeline is still relying on reps to remember every follow-up.</h3>
                <p style={{ fontFamily: I, fontSize: 14.5, color: 'rgba(255,255,255,.80)', lineHeight: 1.65, margin: 0 }}>
                  Find out how many deals your pipeline could stop losing to silence.
                </p>
              </div>
              <Link href="/contact" className="mid-cta-btn"
                ...
                Audit My Sales Pipeline <i className="fa-solid fa-arrow-right" style={{ fontSize: 11 }} />
              </Link>
            </div>
          </div>
          <style>{`
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Why Businesses Choose Isuremedia for Pipeline Automation</h2>
        ...
        <section className="spa-section" style={{ padding: '100px 0', background: '#fff' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 64 }}>
              <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,44px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', margin: '0 0 14px' }}>
                Audit to a Self-Moving Pipeline in <span style={{ color: 'var(--ism-amber)' }}>Five Steps</span>
              </h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75, margin: 0 }}>
                We map, build, test, and document every automation so your team can manage it confidently long after handover.
              </p>
            </div>
            <div className="spa-timeline" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 0, position: 'relative' }}>
          ...
          <style>{`
            @media (max-width: 900px) {
              .spa-timeline { grid-template-columns: 1fr !important; gap: 40px !important; }
              .spa-timeline > *:not(:last-child)::after { content:''; display:block; width:2px; height:32px; background:var(--ism-amber); margin:20px auto 0; }
            }
          `}</style>
        </section>
