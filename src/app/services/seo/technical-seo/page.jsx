...
const BRIDGE_ITEMS = [
  'Full technical SEO audits covering every ranking signal',
  ...
  'Mobile-first and HTTPS compliance',
];

const WHY_MATTERS = [
  { icon: 'fa-solid fa-list-check', title: 'Better rankings across every page', desc: 'Duplicate content, crawl errors, and broken redirects actively suppress rankings. Fixing them removes the ceiling on what your content can achieve.' },
  { icon: 'fa-solid fa-gauge-high', title: 'Faster website, lower bounce rate', desc: '53% of mobile users leave a page that takes more than three seconds to load. Core Web Vitals is a confirmed ranking signal.' },
  { icon: 'fa-solid fa-magnifying-glass', title: 'More pages indexed and ranking', desc: 'If Google cannot crawl your pages, they do not exist in search. Technical SEO makes sure every page you want ranking is being found.' },
  { icon: 'fa-solid fa-robot', title: 'Visibility in AI search results', desc: 'AI systems pull from sites with clean structure, proper schema, and clear entity signals. Technical SEO feeds AI search visibility directly.' },
  { icon: 'fa-solid fa-layer-group', title: 'A stronger base for everything else', desc: 'Content, links, and local SEO all perform better on a technically sound site. Technical SEO multiplies the return on every other investment.' },
  { icon: 'fa-solid fa-shield-halved', title: 'Protection from algorithm updates', desc: 'Sites with clean technical foundations are far less vulnerable to Google changes. Most ranking drops trace back to issues already there.' },
];
...
const COVERAGE = [
  { icon: 'fa-solid fa-spider', title: 'Crawlability & Indexation', impact: 'Foundation', desc: 'We check robots.txt, XML sitemaps, crawl budget, and indexation so every page you want ranking is seen, and every page you don’t is properly blocked.' },
  { icon: 'fa-solid fa-gauge-high', title: 'Site Speed & Core Web Vitals', impact: 'Highest impact', desc: 'We audit and improve load times, server response, image optimisation, and render-blocking resources so your site meets Google’s performance standards.' },
  { icon: 'fa-solid fa-sitemap', title: 'Site Architecture & Internal Linking', impact: 'High impact', desc: 'We map your architecture, fix orphaned pages, and make sure your most important pages get the crawl priority they deserve.' },
  { icon: 'fa-solid fa-code', title: 'Structured Data & Schema Markup', impact: 'Growing fast', desc: 'Schema tells Google and AI systems exactly what your content is about, qualifying you for rich results and AI-generated answer inclusion.' },
  { icon: 'fa-solid fa-clone', title: 'Duplicate Content & Canonicalisation', impact: 'Medium impact', desc: 'We audit for duplication, implement canonical tags correctly, and consolidate authority onto the right pages.' },
  { icon: 'fa-solid fa-mobile-screen', title: 'Mobile-First Compliance', impact: 'High impact', desc: 'Google indexes your mobile site first. We fix usability, layout, and performance issues so your site performs on every device.' },
  { icon: 'fa-solid fa-lock', title: 'HTTPS & Security Signals', impact: 'Standard requirement', desc: 'Mixed content errors, expired certificates, and insecure pages all send negative signals. We make sure your site is fully secure.' },
  { icon: 'fa-solid fa-arrows-turn-right', title: 'Redirect Management', impact: 'Medium impact', desc: 'Broken redirects and chains waste crawl budget and lose ranking authority. We audit and fix every redirect causing problems.' },
];
  ...
  return (
    <section style={{ padding: '100px 0', background: 'var(--color-bg-soft)' }}>
      <div className="ism-container">
        <div className="tseo-faq-grid" style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 64, alignItems: 'start' }}>
          <div className="tseo-faq-sticky" style={{ position: 'sticky', top: 100 }}>
            <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,40px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', marginBottom: 14, marginTop: 0, lineHeight: 1.15 }}>
              Questions About <span style={{ color: 'var(--ism-amber)' }}>Technical SEO</span>
            </h2>
      ...
      <style>{`
        @media (max-width: 900px) {
          .tseo-faq-grid { grid-template-columns: minmax(0,1fr) !important; gap: 32px !important; }
          .tseo-faq-sticky { position: static !important; }
        }
      `}</style>
    </section>
  );
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* ══ 01. HERO ══════════════════════════════════════════════ */}
        <section className="tseo-hero" style={{ background: 'linear-gradient(160deg,var(--ism-blue-50) 0%,#fff 60%)', padding: '88px 0 96px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: '-15%', right: '-8%', width: 720, height: 720, background: 'radial-gradient(circle,rgba(30,77,195,.07) 0%,transparent 65%)', pointerEvents: 'none' }} />

          <div className="ism-container">
            <div className="tseo-hero-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.95fr)', gap: 56, alignItems: 'center', position: 'relative', zIndex: 1 }}>
              ...
              <div>
                <h1 style={{ fontFamily: J, fontWeight: 900, fontSize: 'clamp(30px,3.8vw,54px)', color: 'var(--color-navy)', lineHeight: 1.14, letterSpacing: '-0.5px', marginBottom: 22 }}>
                  If Google Can&apos;t Read Your Site, It Can&apos;t{' '}
              ...
              <div className="tseo-hero-photo" style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', top: 26, right: -20, background: '#fff', borderRadius: 14, padding: '12px 18px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 10 }}>
                  <i className="fa-solid fa-spider" style={{ color: 'var(--ism-amber)', fontSize: 14 }} />
                  <span style={{ fontFamily: J, fontSize: 13, fontWeight: 700, color: 'var(--color-navy)' }}>100% Crawlable</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: 28, left: -24, background: '#fff', borderRadius: 14, padding: '12px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)' }}>
                  ...
                  <span style={{ fontFamily: I, fontSize: 11, fontWeight: 600, color: 'var(--color-text-muted)' }}>Sites Audited &amp; Fixed</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: -16, right: 12, background: '#fff', borderRadius: 14, padding: '10px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 12, fontWeight: 700, color: '#0E9B6E' }}>Core Web Vitals Pass</span>
                </div>
              </div>
          ...
          <style>{`
            @media (max-width: 900px) {
              .tseo-hero-grid { grid-template-columns: minmax(0,1fr) !important; gap: 60px !important; }
              .tseo-hero-photo { margin: 0 12px 24px; }
            }
            @media (max-width: 480px) {
              .tseo-hero { padding: 48px 0 64px !important; }
            }
          `}</style>
        </section>
        ...
        {/* ══ 04. WHAT IS TECHNICAL SEO ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div className="tseo-whatis-box" style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 24, padding: '56px 56px', boxShadow: '0 24px 64px rgba(0,35,83,.08)' }}>
            <div className="tseo-whatis-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.85fr)', gap: 64, alignItems: 'center' }}>
              <div>
                <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', lineHeight: 1.22, letterSpacing: '-0.4px', marginBottom: 20 }}>
                  Make Your Website Easy for Google to Find, Read, and Rank.
                </h2>
              ...
              <div className="tseo-score-wrap" style={{ position: 'relative', height: 340, borderRadius: 20, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'visible' }}>
                ...
                <div style={{ position: 'relative', width: 168, height: 168, borderRadius: '50%', background: 'conic-gradient(var(--ism-amber) 0deg 338deg, var(--ism-blue-100) 338deg 360deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 14px 38px rgba(0,35,83,.14)' }}>
                  <div style={{ width: 134, height: 134, borderRadius: '50%', background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <span style={{ fontFamily: J, fontSize: 10, fontWeight: 700, color: 'var(--color-text-muted)', letterSpacing: '.05em', textTransform: 'uppercase', marginTop: 4 }}>Site Health Score</span>
                  </div>
                </div>

                <div className="tseo-score-badge" style={{ position: 'absolute', top: 18, left: 0, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-spider" style={{ color: 'var(--color-primary)', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Crawl Health</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Clean</div>
                  </div>
                </div>

                <div className="tseo-score-badge" style={{ position: 'absolute', top: 30, right: -6, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'rgba(255,176,0,.16)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-code" style={{ color: 'var(--ism-amber)', fontSize: 12 }} />
                  </div>
                  <div>
                ...
                <div className="tseo-score-badge" style={{ position: 'absolute', bottom: 8, left: -10, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Core Web Vitals</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Pass</div>
                  </div>
                </div>

                <div className="tseo-score-badge" style={{ position: 'absolute', bottom: 24, right: 4, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Optimised</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <style>{`
            @media(max-width:860px){ .tseo-whatis-grid{ grid-template-columns:minmax(0,1fr) !important; gap:40px !important; } .tseo-whatis-grid > div:last-child{ order:-1; } }
            @media(max-width:640px){ .tseo-whatis-box{ padding:32px 24px !important; } }
            @media(max-width:500px){ .tseo-score-badge{ padding:8px 10px !important; gap:7px !important; } .tseo-score-badge > div:first-child{ width:24px !important; height:24px !important; } }
          `}</style>
        </section>

        {/* ══ 05. WHY TECHNICAL SEO MATTERS ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 18 }}>A Site Google Cannot Read Is a Site That Does Not Rank.</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.78 }}>
                60% of searches in 2026 are answered directly by AI overviews and featured snippets. The businesses that appear have one thing in common: technically clean, well-structured sites.
              </p>
            </div>
        ...
        {/* ══ 06. WHAT TECHNICAL SEO COVERS ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 18 }}>What Technical SEO Actually Covers.</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.78 }}>
                A set of overlapping systems that together determine whether Google can find, read, trust, and rank your website. Every item below is something we actively work on.
              </p>
            </div>
        ...
        {/* ══ 07. OUR TECHNICAL SEO SERVICES ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Our Technical SEO Services</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>Fix What Is Holding Your Rankings Back, From the Ground Up.</p>
            </div>
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Who Technical SEO Is Built For</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>If Your Site Has Rankings to Protect or Traffic to Recover, This Is for You.</p>
            </div>
          ...
          <div className="ism-container">
            <div className="mid-cta" style={{ background: 'var(--color-primary)', borderRadius: 20, padding: '40px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', maxWidth: 560 }}>
                <h3 style={{ fontFamily: J, fontSize: 'clamp(18px,2vw,24px)', fontWeight: 800, color: '#fff', marginBottom: 8, lineHeight: 1.3 }}>Your rankings might be lower than they should be right now.</h3>
                <p style={{ fontFamily: I, fontSize: 14.5, color: 'rgba(255,255,255,.80)', lineHeight: 1.65, margin: 0 }}>
                  A free technical SEO audit will show you exactly why.
                </p>
              </div>
              <Link href="/contact" className="mid-cta-btn"
                ...
                Run My Free Technical SEO Audit <i className="fa-solid fa-arrow-right" style={{ fontSize: 11 }} />
              </Link>
            </div>
          </div>
          <style>{`
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Why Businesses Choose Isuremedia for Technical SEO</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>Deep Technical Work. Plain English Reporting.</p>
            </div>
            <div className="why-ism-bento" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16 }}>
              ...
              <div className="why-ism-bento-row" style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                ...
                <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', minHeight: 200 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img loading="lazy" src="\services-mid-image\seo.webp" alt="What makes Isuremedia different" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                </div>
        ...
        {/* ══ 11. OUR PROCESS ══════════════════════════════════════════════ */}
        <section className="tseo-section" style={{ padding: '100px 0', background: '#fff' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 64 }}>
              <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,44px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', margin: '0 0 14px' }}>
                How ISureMedia Delivers <span style={{ color: 'var(--ism-amber)' }}>Technical SEO Results</span>
              </h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75, margin: 0 }}>
                Audit First. Fix What Matters. Keep It Clean.
              </p>
            </div>
            <div className="tseo-timeline" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 0, position: 'relative' }}>
            ...
            <p style={{ textAlign: 'center', fontFamily: I, fontSize: 14, color: 'var(--color-text-muted)', marginTop: 40 }}>
              Crawl and indexation improvements visible in two to four weeks in Search Console. Ranking improvements from technical fixes typically appear within six to twelve weeks.
            </p>
          ...
          <style>{`
            @media (max-width: 900px) {
              .tseo-timeline { grid-template-columns: 1fr !important; gap: 40px !important; }
              .tseo-timeline > *:not(:last-child)::after { content:''; display:block; width:2px; height:32px; background:var(--ism-amber); margin:20px auto 0; }
            }
          `}</style>
        </section>
        ...
        <section style={{ padding: '96px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <p style={{ fontFamily: I, fontSize: 15, color: 'var(--color-text-muted)', marginTop: 10 }}>A few things that go well with a technically sound site.</p>
            </div>
            <div className="tseo-related-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
              {RELATED.map((r, i) => (
          ...
          <style>{`@media(max-width:860px){ .tseo-related-grid{ grid-template-columns:1fr !important; } }`}</style>
        </section>
