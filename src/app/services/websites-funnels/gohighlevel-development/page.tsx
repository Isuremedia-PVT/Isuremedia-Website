...
const BRIDGE_ITEMS = [
  'Custom GoHighLevel website and funnel design',
  'Sub-account setup and CRM configuration',
  'Pipeline, calendar, and automation build',
  'Custom snapshot creation for repeatable rollouts',
  'Payment and form integration',
  'API and third-party tool connections',
  'Ongoing build support and optimisation',
];
...
const WHY_MATTERS = [
  ...
  { icon: 'fa-solid fa-gears', title: 'Deep customisation for agencies and businesses alike', desc: 'Custom menus, values, and workflows configured to match exactly how a business or agency operates.' },
];
...
const FAILURE_MODES = [
  { icon: 'fa-solid fa-box-open', title: 'Built from a generic template', impact: 'Most common', desc: 'A stock GoHighLevel template rarely matches the brand or the sales process. It ends up half-used and underperforming.' },
  { icon: 'fa-solid fa-diagram-project', title: 'Pipelines that don’t match how the business sells', impact: 'High impact', desc: 'Stages copied from a default snapshot create friction on every deal instead of reflecting the actual sales process.' },
  { icon: 'fa-solid fa-plug-circle-xmark', title: 'Automations that were never properly tested', impact: 'High impact', desc: 'A workflow that looks right on paper but fails on an edge case erodes trust in the whole system fast.' },
  { icon: 'fa-solid fa-user-graduate', title: 'No training for the team using it', impact: 'Ongoing', desc: 'A powerful system nobody knows how to use ends up running at a fraction of its potential.' },
];
...
const SERVICES = [
  ...
  { icon: 'fa-solid fa-copy', title: 'Custom Snapshot Creation', desc: 'A repeatable, proven system built once and deployed to new sub-accounts or clients quickly.' },
  { icon: 'fa-solid fa-credit-card', title: 'Payments & Form Integration', desc: 'Payment processors and forms connected properly so data flows straight into the CRM.' },
  { icon: 'fa-solid fa-plug', title: 'API & Third-Party Integrations', desc: 'Connections to tools outside the GHL ecosystem where a native integration doesn’t exist.' },
  { icon: 'fa-solid fa-headset', title: 'Ongoing Build Support', desc: 'Continued configuration help and troubleshooting as your funnels and automations evolve.' },
];
...
const WHY_ISM = [
  { icon: 'fa-solid fa-pen-ruler', title: 'We build custom, not from a generic template', desc: 'Every site, funnel, and pipeline configured around your brand and your actual sales process.' },
  { icon: 'fa-solid fa-diagram-project', title: 'We configure pipelines around how you sell', desc: 'Stages, fields, and tags built to match the real sales process, not a default snapshot.' },
  { icon: 'fa-solid fa-flask', title: 'We test every automation before it goes live', desc: 'Workflows tested against edge cases, not just the happy path, before the client ever sees them.' },
  { icon: 'fa-solid fa-copy', title: 'We build repeatable systems where it makes sense', desc: 'Snapshots built properly so agencies can deploy proven systems to new clients quickly.' },
...
const PROCESS = [
  { n: '01', title: 'Discovery & Process Mapping', desc: 'We map your sales process, funnel goals, and automation needs before configuring anything.' },
  { n: '02', title: 'Website & Funnel Design', desc: 'Custom design delivered for review and approval before development begins.' },
  { n: '03', title: 'CRM & Pipeline Build', desc: 'Pipelines, custom fields, and tagging configured to match the mapped sales process.' },
  { n: '04', title: 'Automation & Workflow Build', desc: 'Triggers and workflows built and tested against real scenarios before launch.' },
  { n: '05', title: 'QA & Launch', desc: 'Full testing across the funnel, forms, and payment flow before going live.' },
  { n: '06', title: 'Ongoing Support & Optimisation', desc: 'Continued configuration support as your funnels, offers, and automations evolve.' },
];
...
const FAQS = [
  ...
  { q: 'Can you fix a GoHighLevel account that was never properly configured?', a: 'Yes. Most GHL frustration comes from an incomplete or generic build, not the platform. We audit first and recommend the right level of work.' },
  { q: 'Can GoHighLevel integrate with tools outside its ecosystem?', a: 'Yes, through native integrations, Zapier or Make, or custom API connections we build directly.' },
  { q: 'Do you offer ongoing support after the build?', a: 'Yes, on a month-to-month basis for continued configuration changes, troubleshooting, and optimisation.' },
  { q: 'Is GoHighLevel a good fit for a small local business?', a: 'Yes, especially for service businesses needing booking, follow-up, and reputation management in one connected system.' },
];
  ...
  return (
    <section style={{ padding: '100px 0', background: 'var(--color-bg-soft)' }}>
      <div className="ism-container">
        <div className="ghldev-faq-grid" style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 64, alignItems: 'start' }}>
          <div className="ghldev-faq-sticky" style={{ position: 'sticky', top: 100 }}>
            <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,40px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', marginBottom: 14, marginTop: 0, lineHeight: 1.15 }}>
              Questions About <span style={{ color: 'var(--ism-amber)' }}>GoHighLevel Development</span>
            </h2>
      ...
      <style>{`
        @media (max-width: 900px) {
          .ghldev-faq-grid { grid-template-columns: minmax(0,1fr) !important; gap: 32px !important; }
          .ghldev-faq-sticky { position: static !important; }
        }
      `}</style>
    </section>
  );
...
export default function GoHighLevelDevelopmentPage() {
  return (
    <>
      <Navbar />
      <main>
        ...
        {/* ══ 01. HERO ══════════════════════════════════════════════ */}
        <section className="ghldev-hero" style={{ background: 'linear-gradient(160deg,var(--ism-blue-50) 0%,#fff 60%)', padding: '88px 0 96px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: '-15%', right: '-8%', width: 720, height: 720, background: 'radial-gradient(circle,rgba(30,77,195,.07) 0%,transparent 65%)', pointerEvents: 'none' }} />

          <div className="ism-container">
            <div className="ghldev-hero-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.95fr)', gap: 56, alignItems: 'center', position: 'relative', zIndex: 1 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
                  <a href="/contact"
                    ...
                    Get a Free GoHighLevel Consultation
                  </a>
                </div>
              </div>
              ...
              <div className="ghldev-hero-photo" style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', top: 26, right: -20, background: '#fff', borderRadius: 14, padding: '12px 18px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 10 }}>
                  <i className="fa-solid fa-copy" style={{ color: 'var(--ism-amber)', fontSize: 14 }} />
                  <span style={{ fontFamily: J, fontSize: 13, fontWeight: 700, color: 'var(--color-navy)' }}>Repeatable Snapshots</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: 28, left: -24, background: '#fff', borderRadius: 14, padding: '12px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)' }}>
                  ...
                  <span style={{ fontFamily: I, fontSize: 11, fontWeight: 600, color: 'var(--color-text-muted)' }}>Systems Live &amp; Automated</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: -16, right: 12, background: '#fff', borderRadius: 14, padding: '10px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 12, fontWeight: 700, color: '#0E9B6E' }}>Automation Tested</span>
                </div>
              </div>
          ...
          <style>{`
            @media (max-width: 900px) {
              .ghldev-hero-grid { grid-template-columns: minmax(0,1fr) !important; gap: 60px !important; }
              .ghldev-hero-photo { margin: 0 12px 24px; }
            }
            @media (max-width: 480px) {
              .ghldev-hero { padding: 48px 0 64px !important; }
            }
          `}</style>
        </section>
            ...
            <div className="bridge-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,0.9fr)', gap: 64, alignItems: 'start' }}>
              <div>
                <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(24px,2.8vw,40px)', color: 'var(--color-navy)', lineHeight: 1.2, letterSpacing: '-0.4px', marginBottom: 24 }}>
                  A Generic GoHighLevel Template Is a Half-Used System.
                </h2>
                ...
                <a href="/contact"
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-accent-hover)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'var(--ism-amber)'; e.currentTarget.style.transform = ''; }}>
                  Claim Your Free GoHighLevel Consultation
                </a>
              </div>
        ...
        {/* ══ 04. WHAT IS PROPER GHL DEVELOPMENT ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div className="ghldev-whatis-box" style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 24, padding: '56px 56px', boxShadow: '0 24px 64px rgba(0,35,83,.08)' }}>
            <div className="ghldev-whatis-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.85fr)', gap: 64, alignItems: 'center' }}>
              ...
              <div className="ghldev-score-wrap" style={{ position: 'relative', height: 340, borderRadius: 20, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'visible' }}>
                ...
                <div style={{ position: 'relative', width: 168, height: 168, borderRadius: '50%', background: 'conic-gradient(var(--ism-amber) 0deg 324deg, var(--ism-blue-100) 324deg 360deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 14px 38px rgba(0,35,83,.14)' }}>
                  <div style={{ width: 134, height: 134, borderRadius: '50%', background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <span style={{ fontFamily: J, fontSize: 10, fontWeight: 700, color: 'var(--color-text-muted)', letterSpacing: '.05em', textTransform: 'uppercase', marginTop: 4 }}>Connected System</span>
                  </div>
                </div>

                <div className="ghldev-score-badge" style={{ position: 'absolute', top: 18, left: 0, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Pipeline</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Configured</div>
                  </div>
                </div>

                <div className="ghldev-score-badge" style={{ position: 'absolute', top: 30, right: -6, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Tested</div>
                  </div>
                </div>

                <div className="ghldev-score-badge" style={{ position: 'absolute', bottom: 8, left: -10, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'rgba(30,158,90,.14)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-copy" style={{ color: '#1E9E5A', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Snapshot</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Ready</div>
                  </div>
                </div>

                <div className="ghldev-score-badge" style={{ position: 'absolute', bottom: 24, right: 4, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-graduation-cap" style={{ color: 'var(--color-primary)', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Training</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Included</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <style>{`
            @media(max-width:860px){ .ghldev-whatis-grid{ grid-template-columns:minmax(0,1fr) !important; gap:40px !important; } .ghldev-whatis-grid > div:last-child{ order:-1; } }
            @media(max-width:640px){ .ghldev-whatis-box{ padding:32px 24px !important; } }
            @media(max-width:500px){ .ghldev-score-badge{ padding:8px 10px !important; gap:7px !important; } .ghldev-score-badge > div:first-child{ width:24px !important; height:24px !important; } }
          `}</style>
        </section>
        ...
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 18 }}>Everything Your Front Office Needs, in One System.</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.78 }}>
                Website, CRM, calendar, and automation working from the same data, properly configured.
              </p>
            </div>
            ...
            <div style={{ textAlign: 'center' }}>
              <a href="/contact"
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-accent-hover)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'var(--ism-amber)'; e.currentTarget.style.transform = ''; }}>
                Start Your GoHighLevel Build Today
              </a>
            </div>
          </div>
          <style>{`
        ...
        {/* ══ 06. WHAT CAUSES GHL BUILDS TO FAIL ══════════════════════════════════════════════ */}
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Who GoHighLevel Development Is Built For</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>If You Want One System That Actually Works Together, This Is It.</p>
            </div>
          ...
          <div className="ism-container">
            <div className="mid-cta" style={{ background: 'var(--color-primary)', borderRadius: 20, padding: '40px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', maxWidth: 560 }}>
                <h3 style={{ fontFamily: J, fontSize: 'clamp(18px,2vw,24px)', fontWeight: 800, color: '#fff', marginBottom: 8, lineHeight: 1.3 }}>Find out what a properly configured GoHighLevel system could do for you.</h3>
              ...
              <a href="/contact" className="mid-cta-btn"
                ...
                Book My Free GoHighLevel Consultation <i className="fa-solid fa-arrow-right" style={{ fontSize: 11 }} />
              </a>
            </div>
          </div>
          <style>{`
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              ...
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>Custom Built. Properly Tested. Actually Adopted.</p>
            </div>
            <div className="why-ism-bento" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16 }}>
              ...
              <div className="why-ism-bento-row" style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                ...
                <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', minHeight: 200 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src="https://picsum.photos/seed/ghldevwhatmakesdifferent/700/620" alt="What makes Isuremedia different" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                </div>
        ...
        {/* ══ 11. OUR PROCESS ══════════════════════════════════════════════ */}
        <section className="ghldev-section" style={{ padding: '100px 0', background: '#fff' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 64 }}>
              <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,44px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', margin: '0 0 14px' }}>
                How ISureMedia Builds <span style={{ color: 'var(--ism-amber)' }}>GoHighLevel Systems</span>
              </h2>
            ...
            <div className="ghldev-timeline" style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 0, position: 'relative' }}>
            ...
            <p style={{ textAlign: 'center', fontFamily: I, fontSize: 14, color: 'var(--color-text-muted)', marginTop: 40 }}>
              A standard website and funnel build typically takes two to three weeks. Full CRM and automation builds usually take three to five weeks.
            </p>
          ...
          <style>{`
            @media (max-width: 900px) {
              .ghldev-timeline { grid-template-columns: repeat(3,1fr) !important; gap: 40px !important; }
            }
            @media (max-width: 560px) {
              .ghldev-timeline { grid-template-columns: 1fr !important; }
              .ghldev-timeline > *:not(:last-child)::after { content:''; display:block; width:2px; height:32px; background:var(--ism-amber); margin:20px auto 0; }
            }
          `}</style>
        </section>
        ...
        <section style={{ padding: '96px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <p style={{ fontFamily: I, fontSize: 15, color: 'var(--color-text-muted)', marginTop: 10 }}>A few things that go well with GoHighLevel development.</p>
            </div>
            <div className="ghldev-related-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
              {RELATED.map((r, i) => (
          ...
          <style>{`@media(max-width:860px){ .ghldev-related-grid{ grid-template-columns:1fr !important; } }`}</style>
        </section>
