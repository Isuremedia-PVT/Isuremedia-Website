...
const BRIDGE_ITEMS = [
  'AI feature strategy and technical scoping',
  'Custom AI chatbot and assistant development',
...
const FAILURE_MODES = [
  { icon: 'fa-solid fa-file-circle-question', title: 'AI added without a clear use case', impact: 'Most common', desc: '"We should add AI" without a specific problem to solve produces a feature nobody uses. We scope the use case first.' },
  { icon: 'fa-solid fa-database', title: 'No grounding in real business data', impact: 'High impact', desc: 'A model with no access to your actual data hallucinates confidently. RAG and proper data access fix this.' },
  { icon: 'fa-solid fa-vial', title: 'No evaluation process for quality', impact: 'High impact', desc: 'Without testing against real cases, prompt and model changes can silently degrade output quality.' },
  { icon: 'fa-solid fa-sack-dollar', title: 'No cost architecture', impact: 'Ongoing', desc: 'Unoptimised API usage and unnecessary model calls can turn a useful feature into an expensive one fast.' },
];
...
const SERVICES = [
  ...
  { icon: 'fa-solid fa-pen-to-square', title: 'Prompt Engineering & Model Evaluation', desc: 'Structured testing and refinement so AI outputs stay reliable as the system evolves.' },
  { icon: 'fa-solid fa-plug', title: 'CRM & Data Integration', desc: 'AI features connected to your actual business data and systems, not operating in isolation.' },
  { icon: 'fa-solid fa-headset', title: 'Ongoing Monitoring & Maintenance', desc: 'Performance and cost monitored continuously, with adjustments as models and usage patterns evolve.' },
];
...
const WHY_ISM = [
  ...
  { icon: 'fa-solid fa-comments', title: 'Plain English documentation', desc: 'How the AI feature works, what it can and can’t do, and how to make future changes.' },
...
const PROCESS = [
  { n: '01', title: 'Discovery & Use Case Scoping', desc: 'We identify where AI genuinely adds value and define the specific problem it will solve.' },
  { n: '02', title: 'Architecture & Data Design', desc: 'Data access, grounding strategy, and integration points designed before development begins.' },
  { n: '03', title: 'Development & Prompt Engineering', desc: 'The AI feature built and refined through structured prompt testing and evaluation.' },
  { n: '04', title: 'Integration & Testing', desc: 'Connected to your existing systems and tested against real use cases before launch.' },
  { n: '05', title: 'Launch & Monitoring Setup', desc: 'The feature goes live with performance and cost monitoring in place from day one.' },
  { n: '06', title: 'Ongoing Evaluation & Maintenance', desc: 'Continued monitoring, evaluation, and refinement as usage patterns and models evolve.' },
];
  ...
  return (
    <section style={{ padding: '100px 0', background: 'var(--color-bg-soft)' }}>
      <div className="ism-container">
        <div className="ai-faq-grid" style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 64, alignItems: 'start' }}>
          <div className="ai-faq-sticky" style={{ position: 'sticky', top: 100 }}>
            <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,40px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', marginBottom: 14, marginTop: 0, lineHeight: 1.15 }}>
              Questions About <span style={{ color: 'var(--ism-amber)' }}>AI Development</span>
            </h2>
      ...
      <style>{`
        @media (max-width: 900px) {
          .ai-faq-grid { grid-template-columns: minmax(0,1fr) !important; gap: 32px !important; }
          .ai-faq-sticky { position: static !important; }
        }
      `}</style>
    </section>
  );
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* ══ 01. HERO ══════════════════════════════════════════════ */}
        <section className="ai-hero" style={{ background: 'linear-gradient(160deg,var(--ism-blue-50) 0%,#fff 60%)', padding: '88px 0 96px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: '-15%', right: '-8%', width: 720, height: 720, background: 'radial-gradient(circle,rgba(30,77,195,.07) 0%,transparent 65%)', pointerEvents: 'none' }} />

          <div className="ism-container">
            <div className="ai-hero-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.95fr)', gap: 56, alignItems: 'center', position: 'relative', zIndex: 1 }}>
              ...
              <div className="ai-hero-photo" style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', top: 26, right: -20, background: '#fff', borderRadius: 14, padding: '12px 18px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 13, fontWeight: 700, color: 'var(--color-navy)' }}>Grounded in Your Data</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: 28, left: -24, background: '#fff', borderRadius: 14, padding: '12px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)' }}>
                  ...
                  <span style={{ fontFamily: I, fontSize: 11, fontWeight: 600, color: 'var(--color-text-muted)' }}>Features Shipped &amp; Evaluated</span>
                </div>
                ...
                <div style={{ position: 'absolute', bottom: -16, right: 12, background: '#fff', borderRadius: 14, padding: '10px 16px', boxShadow: '0 14px 32px rgba(0,35,83,.16)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 12, fontWeight: 700, color: '#0E9B6E' }}>Cost-Optimised Architecture</span>
                </div>
              </div>
          ...
          <style>{`
            @media (max-width: 900px) {
              .ai-hero-grid { grid-template-columns: minmax(0,1fr) !important; gap: 60px !important; }
              .ai-hero-photo { margin: 0 12px 24px; }
            }
            @media (max-width: 480px) {
              .ai-hero { padding: 48px 0 64px !important; }
            }
          `}</style>
        </section>
            ...
            <div className="bridge-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,0.9fr)', gap: 64, alignItems: 'start' }}>
              <div>
                ...
                <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.82, marginBottom: 32 }}>
                  ISureMedia{' '}
                  scopes the actual use case first, then builds AI grounded in your real data, tested, monitored, and built to keep working.
                </p>
                <Link href="/contact"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '15px 32px', borderRadius: 8, fontFamily: J, fontSize: 14, fontWeight: 700, color: 'var(--color-navy)', background: 'var(--ism-amber)', textDecoration: 'none', letterSpacing: '.04em', textTransform: 'uppercase', boxShadow: '0 6px 20px rgba(255,176,0,.35)', transition: 'all .18s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-accent-hover)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
        ...
        {/* ══ 04. WHAT IS PROPER AI DEVELOPMENT ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div className="ai-whatis-box" style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 24, padding: '56px 56px', boxShadow: '0 24px 64px rgba(0,35,83,.08)' }}>
            <div className="ai-whatis-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.85fr)', gap: 64, alignItems: 'center' }}>
              <div>
                <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', lineHeight: 1.22, letterSpacing: '-0.4px', marginBottom: 20 }}>
                  A Feature, Not a Gimmick. Grounded, Not Guessing.
                </h2>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
                    RAG systems{' '}
                    ground AI responses in your actual data, not just what a general model happens to know.
                  </p>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
                    Proper integration means{' '}
                    AI that reads and writes to your systems, not a standalone chatbot isolated from your business.
                  </p>
                  <p style={{ fontFamily: I, fontSize: 15.5, color: 'var(--color-text-muted)', lineHeight: 1.85, margin: 0 }}>
                    Development means scoping the use case first, then building, testing, and monitoring it like any other business-critical feature.
                  </p>
                </div>
              </div>
              <div className="ai-score-wrap" style={{ position: 'relative', height: 340, borderRadius: 20, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'visible' }}>
                <div aria-hidden style={{ position: 'absolute', width: 260, height: 260, borderRadius: '50%', border: '1px dashed var(--ism-blue-100)' }} />

                <div style={{ position: 'relative', width: 168, height: 168, borderRadius: '50%', background: 'conic-gradient(var(--ism-amber) 0deg 318deg, var(--ism-blue-100) 318deg 360deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 14px 38px rgba(0,35,83,.14)' }}>
                  <div style={{ width: 134, height: 134, borderRadius: '50%', background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontFamily: J, fontSize: 34, fontWeight: 900, color: 'var(--color-navy)', lineHeight: 1 }}>RAG</span>
                    <span style={{ fontFamily: J, fontSize: 10, fontWeight: 700, color: 'var(--color-text-muted)', letterSpacing: '.05em', textTransform: 'uppercase', marginTop: 4 }}>Grounded Answers</span>
                  </div>
                </div>

                <div className="ai-score-badge" style={{ position: 'absolute', top: 18, left: 0, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-magnifying-glass-chart" style={{ color: 'var(--color-primary)', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Use Case</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Scoped</div>
                  </div>
                </div>

                <div className="ai-score-badge" style={{ position: 'absolute', top: 30, right: -6, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Evaluation</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Ongoing</div>
                  </div>
                </div>

                <div className="ai-score-badge" style={{ position: 'absolute', bottom: 8, left: -10, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'rgba(30,158,90,.14)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-sack-dollar" style={{ color: '#1E9E5A', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Cost</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: '#1E9E5A' }}>Optimised</div>
                  </div>
                </div>

                <div className="ai-score-badge" style={{ position: 'absolute', bottom: 24, right: 4, display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 12, padding: '10px 14px', boxShadow: '0 10px 26px rgba(0,35,83,.16)' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--ism-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <i className="fa-solid fa-plug" style={{ color: 'var(--color-primary)', fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: J, fontSize: 13, fontWeight: 800, color: 'var(--color-navy)', lineHeight: 1.2 }}>Integrated</div>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)' }}>Not Standalone</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <style>{`
            @media(max-width:860px){ .ai-whatis-grid{ grid-template-columns:minmax(0,1fr) !important; gap:40px !important; } .ai-whatis-grid > div:last-child{ order:-1; } }
            @media(max-width:640px){ .ai-whatis-box{ padding:32px 24px !important; } }
            @media(max-width:500px){ .ai-score-badge{ padding:8px 10px !important; gap:7px !important; } .ai-score-badge > div:first-child{ width:24px !important; height:24px !important; } }
          `}</style>
        </section>

        {/* ══ 05. WHY AI DEVELOPMENT MATTERS ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
        ...
        {/* ══ 06. WHAT CAUSES AI PROJECTS TO FAIL ══════════════════════════════════════════════ */}
        ...
        {/* ══ 07. OUR AI DEVELOPMENT SERVICES ══════════════════════════════════════════════ */}
        <section style={{ padding: '104px 0', background: '#fff' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Our AI Development Services</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>Strategy, Development, Integration, and Ongoing Evaluation.</p>
            </div>
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Who AI Development Is Built For</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>If You Have a Real Problem AI Could Solve, This Is Where to Start.</p>
            </div>
          ...
          <div className="ism-container">
            <div className="mid-cta" style={{ background: 'var(--color-primary)', borderRadius: 20, padding: '40px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', maxWidth: 560 }}>
                <h3 style={{ fontFamily: J, fontSize: 'clamp(18px,2vw,24px)', fontWeight: 800, color: '#fff', marginBottom: 8, lineHeight: 1.3 }}>Find out where AI could genuinely save your team time.</h3>
              ...
              <Link href="/appointment" className="mid-cta-btn"
                ...
                Book My Free AI Strategy Session <i className="fa-solid fa-arrow-right" style={{ fontSize: 11 }} />
              </Link>
            </div>
          </div>
          <style>{`
        ...
        <section style={{ padding: '104px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 56px' }}>
              <h2 style={{ fontFamily: J, fontWeight: 800, fontSize: 'clamp(22px,2.6vw,38px)', color: 'var(--color-navy)', letterSpacing: '-0.4px', marginBottom: 14 }}>Why Businesses Choose Isuremedia for AI Development</h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75 }}>Scoped Properly. Grounded in Data. Actually Evaluated.</p>
            </div>
        ...
        {/* ══ 11. OUR PROCESS ══════════════════════════════════════════════ */}
        <section className="ai-section" style={{ padding: '100px 0', background: '#fff' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
            <div style={{ textAlign: 'center', marginBottom: 64 }}>
              <h2 style={{ fontFamily: J, fontSize: 'clamp(26px,3vw,44px)', fontWeight: 900, color: 'var(--color-navy)', letterSpacing: '-0.5px', margin: '0 0 14px' }}>
                How ISureMedia Builds <span style={{ color: 'var(--ism-amber)' }}>AI Features</span>
              </h2>
              <p style={{ fontFamily: I, fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.75, margin: 0 }}>
                Discover. Architect. Build. Integrate. Launch. Evaluate.
              </p>
            </div>
            <div className="ai-timeline" style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 0, position: 'relative' }}>
            ...
            <p style={{ textAlign: 'center', fontFamily: I, fontSize: 14, color: 'var(--color-text-muted)', marginTop: 40 }}>
              A focused chatbot or workflow automation might take three to five weeks. RAG systems and deeper integrations typically take longer.
            </p>
          ...
          <style>{`
            @media (max-width: 900px) {
              .ai-timeline { grid-template-columns: repeat(3,1fr) !important; gap: 40px !important; }
            }
            @media (max-width: 560px) {
              .ai-timeline { grid-template-columns: 1fr !important; }
              .ai-timeline > *:not(:last-child)::after { content:''; display:block; width:2px; height:32px; background:var(--ism-amber); margin:20px auto 0; }
            }
          `}</style>
        </section>
        ...
        <section style={{ padding: '96px 0', background: 'var(--color-bg-soft)' }}>
          <div className="ism-container">
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <p style={{ fontFamily: I, fontSize: 15, color: 'var(--color-text-muted)', marginTop: 10 }}>A few things that go well with custom AI development.</p>
            </div>
            <div className="ai-related-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
              {RELATED.map((r, i) => (
          ...
          <style>{`@media(max-width:860px){ .ai-related-grid{ grid-template-columns:1fr !important; } }`}</style>
        </section>
