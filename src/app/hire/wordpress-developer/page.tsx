...
const whatYouGet = [
  { icon: 'fa-solid fa-code', title: 'Senior-Level WordPress Developers', desc: 'Every developer we place has been vetted through technical assessments and real project experience. No juniors, no guesswork.' },
  { icon: 'fa-solid fa-bolt', title: 'Fast Onboarding — Start in 48h', desc: 'Skip months of hiring. Tell us what you need, we match you with the right developer, and they are ready to ship code within 48 hours.' },
  { icon: 'fa-solid fa-shield-halved', title: 'Fully Managed & Accountable', desc: 'We handle contracts, NDA, time tracking, and quality reviews so you get the output without the management overhead.' },
];

const engagements = [
  { title: 'Part-Time', hours: '20 hrs/week', desc: 'Perfect for ongoing maintenance, small feature builds, or when you need consistent support without full commitment.', price: 'From $800/mo' },
  { title: 'Full-Time', hours: '40 hrs/week', desc: 'A dedicated developer embedded in your team — ideal for active product builds, large migrations, or scaling your agency.', price: 'From $1,400/mo' },
  { title: 'Project-Based', hours: 'Fixed scope', desc: 'Need a specific deliverable completed? We quote it, scope it, and deliver it. No ongoing retainer required.', price: 'Custom quote' },
];

const whyISM = [
  { icon: 'fa-solid fa-user-check', title: 'Thoroughly Vetted', desc: 'Every developer passes a 3-stage vetting process including technical tests, portfolio review, and communication assessment.' },
  { icon: 'fa-solid fa-rotate', title: 'Replacement Guarantee', desc: 'Not satisfied? We replace your developer within 5 business days, no questions asked.' },
  { icon: 'fa-solid fa-clock', title: 'Timezone Flexible', desc: 'We have developers across multiple timezones so you get coverage that fits your working hours.' },
  { icon: 'fa-solid fa-comments', title: 'Transparent Communication', desc: 'Daily stand-ups, weekly reports, Slack or email — you stay informed without chasing anyone.' },
];

const steps = [
  { num: '01', title: 'Post Your Brief', desc: 'Share your project requirements, tech stack, and timeline.' },
  { num: '02', title: 'Interview Candidates', desc: 'We shortlist 2-3 vetted developers for you to interview.' },
  { num: '03', title: 'Start Working', desc: 'Onboard your chosen developer and begin immediately.' },
  { num: '04', title: 'Deliver Results', desc: 'Track progress, review milestones, and receive your deliverables.' },
];
      ...
      <section style={{ background: 'var(--color-bg-soft)', padding: '88px 0 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 40px', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--ism-blue-50)', border: '1px solid var(--ism-blue-100)', borderRadius: 100, padding: '6px 16px', marginBottom: 24 }}>
            <i className="fa-brands fa-wordpress" style={{ fontSize: 12, color: 'var(--color-primary)' }} />
          ...
          <h1 style={{ maxWidth: 760, marginBottom: 24 }}>Hire a WordPress Developer — Expert Builds, Fast Delivery</h1>
          <p style={{ fontFamily: I, fontSize: 18, color: 'var(--color-text-muted)', lineHeight: 1.75, maxWidth: 620, marginBottom: 40 }}>
            Stop settling for slow freelancers and costly agencies. Get a vetted, full-time or part-time WordPress developer from Isuremedia — onboarded in 48 hours, fully managed, and built to deliver.
          </p>
          ...
          <div className="skills-inner" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            <div>
              ...
              <p style={{ fontFamily: I, fontSize: 17, color: 'var(--color-text-muted)', lineHeight: 1.75, marginBottom: 32 }}>
                Our WordPress developers are specialists, not generalists. They bring deep expertise across the full WordPress ecosystem so you get clean code, fast sites, and lasting results.
              </p>
              <a href="/contact"
          ...
          <div className="eng-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 32 }}>
            {engagements.map((eng, i) => (
              <div key={eng.title} style={{ borderRadius: 16, padding: '36px 32px', border: i === 1 ? '2px solid var(--color-primary)' : '1px solid var(--color-border)', background: i === 1 ? 'var(--ism-blue-50)' : '#fff', position: 'relative' }}>
                ...
                <a href="/contact"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 24px', borderRadius: 8, fontFamily: J, fontSize: 13, fontWeight: 700, color: i === 1 ? 'var(--color-navy)' : 'var(--color-primary)', background: i === 1 ? 'var(--ism-amber)' : 'transparent', textDecoration: 'none', letterSpacing: '.04em', textTransform: 'uppercase', border: i === 1 ? 'none' : '2px solid var(--color-primary)', transition: 'all .18s' }}>
                  Get Started
                </a>
              </div>
            ))}
          ...
          <div className="process-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 28 }}>
            {steps.map((step, i) => (
              <div key={step.num} style={{ position: 'relative', textAlign: 'center', padding: '32px 24px' }}>
                {i < steps.length - 1 && (
                  <div style={{ position: 'absolute', top: 44, right: '-14%', width: '28%', height: 2, background: 'var(--ism-blue-100)' }} />
                )}
